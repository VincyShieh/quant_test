...
NEAR_MISS_VARIANTS_SHARPE = [
    # 提升 Sharpe 的核心手段：平滑 + 缩小宇宙 + MARKET粗粒度中性化优先（降自相关）
    {"name": "[S]decay=4+MARKET",    "decay": 4,  "truncation": 0.08, "neutralization": "MARKET",    "universe": "TOP3000", "nanHandling": "OFF", "testPeriod": "P5Y0M0D"},
    {"name": "[S]TOP2000+MARKET",    "decay": 0,  "truncation": 0.08, "neutralization": "MARKET",    "universe": "TOP2000", "nanHandling": "OFF", "testPeriod": "P5Y0M0D"},
    {"name": "[S]SECTOR中性化",       "decay": 0,  "truncation": 0.08, "neutralization": "SECTOR",    "universe": "TOP3000", "nanHandling": "OFF", "testPeriod": "P5Y0M0D"},
    {"name": "[S]backfill+MARKET",   "decay": 0,  "truncation": 0.08, "neutralization": "MARKET",    "universe": "TOP3000", "nanHandling": "ON",  "testPeriod": "P5Y0M0D"},
    {"name": "[S]INDUSTRY中性化",     "decay": 4,  "truncation": 0.08, "neutralization": "INDUSTRY",   "universe": "TOP2000", "nanHandling": "OFF", "testPeriod": "P5Y0M0D"},
    {"name": "[S]3年+MARKET",        "decay": 0,  "truncation": 0.08, "neutralization": "MARKET",    "universe": "TOP3000", "nanHandling": "OFF", "testPeriod": "P3Y0M0D"},
]
...
NEAR_MISS_VARIANTS_FITNESS = [
    # 提升 Fitness 的核心手段：严格截断 + 重度平滑 + MARKET优先（降自相关）
    {"name": "[F]trunc=0.05+MARKET",  "decay": 0,  "truncation": 0.05, "neutralization": "MARKET",    "universe": "TOP3000", "nanHandling": "OFF", "testPeriod": "P5Y0M0D"},
    {"name": "[F]decay=6+trunc+MARK", "decay": 6,  "truncation": 0.04, "neutralization": "MARKET",    "universe": "TOP3000", "nanHandling": "OFF", "testPeriod": "P5Y0M0D"},
    {"name": "[F]backfill+SECTOR",    "decay": 0,  "truncation": 0.05, "neutralization": "SECTOR",    "universe": "TOP3000", "nanHandling": "ON",  "testPeriod": "P5Y0M0D"},
    {"name": "[F]TOP2000+MARKET",     "decay": 0,  "truncation": 0.05, "neutralization": "MARKET",    "universe": "TOP2000", "nanHandling": "OFF", "testPeriod": "P5Y0M0D"},
    {"name": "[F]INDUSTRY+截断",      "decay": 4,  "truncation": 0.05, "neutralization": "INDUSTRY",   "universe": "TOP2000", "nanHandling": "ON",  "testPeriod": "P5Y0M0D"},
    {"name": "[F]3年+SECTOR",         "decay": 2,  "truncation": 0.05, "neutralization": "SECTOR",    "universe": "TOP3000", "nanHandling": "OFF", "testPeriod": "P3Y0M0D"},
]
...
D0_NEAR_MISS_VARIANTS_SHARPE = [
    # D0 寻婁: 优先缩小宇宙 + MARKET粗粒度中性化（D0自相关要求更高）
    {"name": "[D0-S]TOP1000+MARKET",    "decay": 0,  "truncation": 0.08, "neutralization": "MARKET",    "universe": "TOP1000", "nanHandling": "OFF", "testPeriod": "P5Y0M0D"},
    {"name": "[D0-S]decay=6+MARKET",    "decay": 6,  "truncation": 0.08, "neutralization": "MARKET",    "universe": "TOP1000", "nanHandling": "OFF", "testPeriod": "P5Y0M0D"},
    {"name": "[D0-S]SECTOR+TOP1000",    "decay": 4,  "truncation": 0.08, "neutralization": "SECTOR",    "universe": "TOP1000", "nanHandling": "OFF", "testPeriod": "P5Y0M0D"},
    {"name": "[D0-S]TOP500+MARKET",     "decay": 6,  "truncation": 0.06, "neutralization": "MARKET",    "universe": "TOP500",  "nanHandling": "OFF", "testPeriod": "P5Y0M0D"},
    {"name": "[D0-S]backfill+MARKET",   "decay": 4,  "truncation": 0.08, "neutralization": "MARKET",    "universe": "TOP1000", "nanHandling": "ON",  "testPeriod": "P5Y0M0D"},
    {"name": "[D0-S]3Y+SECTOR",         "decay": 4,  "truncation": 0.06, "neutralization": "SECTOR",    "universe": "TOP1000", "nanHandling": "OFF", "testPeriod": "P3Y0M0D"},
]
...
D0_NEAR_MISS_VARIANTS_FITNESS = [
    {"name": "[D0-F]trunc=0.04+MARK",  "decay": 0,  "truncation": 0.04, "neutralization": "MARKET",    "universe": "TOP1000", "nanHandling": "OFF", "testPeriod": "P5Y0M0D"},
    {"name": "[D0-F]decay=8+MARKET",   "decay": 8,  "truncation": 0.03, "neutralization": "MARKET",    "universe": "TOP1000", "nanHandling": "OFF", "testPeriod": "P5Y0M0D"},
    {"name": "[D0-F]TOP500+SECTOR",    "decay": 4,  "truncation": 0.04, "neutralization": "SECTOR",    "universe": "TOP500",  "nanHandling": "OFF", "testPeriod": "P5Y0M0D"},
    {"name": "[D0-F]INDUSTRY+1000",    "decay": 4,  "truncation": 0.04, "neutralization": "INDUSTRY",   "universe": "TOP1000", "nanHandling": "OFF", "testPeriod": "P5Y0M0D"},
    {"name": "[D0-F]backfill+MARKET",  "decay": 6,  "truncation": 0.04, "neutralization": "MARKET",    "universe": "TOP1000", "nanHandling": "ON",  "testPeriod": "P5Y0M0D"},
    {"name": "[D0-F]extreme+500",      "decay": 8,  "truncation": 0.03, "neutralization": "MARKET",    "universe": "TOP500",  "nanHandling": "ON",  "testPeriod": "P5Y0M0D"},
]
...
_ALPHA_TEMPLATES = [
    ...
    # ════════════════════════════════════════════════════════════════
    # v6 新增: 低自相关优先骨架 (Low Autocorr First)
    # 策略: ts_delta(差分) 天然自相关低 + market/sector粗粒度 + 短窗口
    # 目标: 从源头降低 SELF_CORRELATION 失败率
    # ════════════════════════════════════════════════════════════════

    # ── S36. ts_delta 差分骨架（自相关远低于 decay_linear）──
    "ts_delta(rank({F1}), {W})",
    "group_rank(ts_delta({F1}, {W}), market)",
    "group_neutralize(ts_delta(rank({F1}), {W}), market)",
    "group_rank(ts_delta({F1}, {W}) - ts_delta({F2}, {W}), market)",
    "group_neutralize(ts_delta(rank({F1}) - rank({F2}), {W}), sector)",
    "rank(ts_delta({F1} / ({F2} + 1e-6), {W}))",
    "group_rank(ts_delta({F1}, {W}) / (ts_std_dev({F1}, {W}) + 1e-6), market)",

    # ── S37. 截面差异骨架（横截面天然去自相关）──
    "rank({F1}) - rank({F2})",
    "group_rank(rank({F1}) - rank({F2}), market)",
    "group_neutralize(rank({F1}) - group_mean({F1}, sector), market)",
    "group_rank({F1} - group_mean({F1}, market), sector)",
    "group_neutralize({F1} / (group_mean(abs({F1}), sector) + 1e-6), market)",

    # ── S38. 短窗口反转/动量（5~15天，信号变化快）──
    "group_rank(ts_delta({F1}, 5) / (ts_delta({F1}, 20) + 1e-6), market)",
    "group_neutralize(ts_zscore(ts_delta({F1}, 5), 10), sector)",
    "group_rank(ts_delta(ts_delta({F1}, 3), 5), market)",
    "group_neutralize(ts_mean(ts_delta({F1}, 1), 5), market)",

    # ── S39. ts_av_diff 均值差骨架（比 decay 自相关低）──
    "group_rank(ts_av_diff({F1}, {W}), market)",
    "group_neutralize(ts_av_diff(rank({F1}), {W}), sector)",
    "group_rank(ts_av_diff({F1}, {W}) - ts_av_diff({F2}, {W}), market)",
    "group_neutralize(ts_av_diff({F1} / ({F2} + 1e-6), {W}), market)",

    # ── S40. 混合低自相关骨架 ──
    "group_rank(ts_delta(group_rank({F1}, market), {W}), sector)",
    "group_neutralize(rank({F1}) - ts_mean(rank({F1}), 5), market)",
    "group_rank(ts_zscore(ts_delta({F1}, {W}), 10), market)",
    "group_neutralize(ts_delta(ts_zscore({F1}, 20), 5), sector)",
]

# ★★★ 骨架工厂扩容：从 40 个手工骨架 → 2200+ 个有金融意义的骨架 ★★★
# ★★★ 冷门算子优先：90% 概率抽冷门骨架（pasteurize/bucket/kth_element/ts_entropy...）★★★
# ★★★ 低自相关骨架采样权重提升：ts_delta/market/sector 骨架 3x 采样权重 ★★★
...
_TEMPLATE_WINDOWS = [3, 5, 8, 10, 15, 20, 30]  # 偏向短窗口：信号变化快，自相关性低
_LONG_WINDOWS = [40, 60, 90, 120]  # 长窗口保留但仅用于长线信号模板
...
# ★ 低自相关骨架标记：这些骨架天然自相关低（ts_delta/market/sector/短窗口），采样权重 3x ★
_LOW_AUTOCORR_SKELETON_MARKERS = ["ts_delta", "ts_av_diff", ", market)", ", sector)",
                                   "rank({F1}) - rank({F2})",
                                   "group_mean", "/ (group_mean"]
def _is_low_autocorr_skeleton(skel: str) -> bool:
    """判断骨架是否属于低自相关类型"""
    return any(marker in skel for marker in _LOW_AUTOCORR_SKELETON_MARKERS)
...
def generate_template_alphas(n: int, wq_fields: list, evaluated_alphas: set = None,
    ...
    while len(results) < n and attempts < n * 15:
        ...
        try:
            ...
            # ★ 低自相关骨架 3x 权重：在反重复采样基础上提升 ts_delta/market/sector 骨架权重 ★
            if _sample_skeleton_func:
                template = _sample_skeleton_func(cold_ratio=0.90)
            elif random.random() < 0.40:  # 40% 概率强制选低自相关骨架
                low_pool = [t for t in _ALPHA_TEMPLATES if _is_low_autocorr_skeleton(t)]
                if low_pool:
                    template = random.choice(low_pool)
                else:
...
def generate_d0_leg(n: int, wq_fields: list, wq_fields_by_category: dict,
    ...
    d0_windows = [3, 5, 10, 15, 20, 30]  # 偏向短窗口：D0延迟=0，短窗口信号变化快，自相关更低
...
def gm_first_order_enhance(naked_signal: str) -> list:
    ...
    - 不同中性化方式 (market/sector/industry/subindustry，粗粒度优先)
    - 符号翻转 (捕获反向)
    ...
    # ts_delta 差分（优先：自相关远低于 decay_linear）
    for d in [5, 10, 20]:
        v = f"ts_delta({naked_signal}, {d})"
        if len(v) <= 150:
            variants.add(v)
    ...
    # 不同中性化：market/sector 优先（粗粒度=低自相关）
    for grp in ['market', 'sector', 'industry', 'subindustry']:
...
def inject_neutralization(base_alpha: str) -> str:
    ...
    策略：优先用粗粒度分组（market/sector），降低自相关。subindustry 太细，
    组内样本少、信号惯性大，是自相关超标的常见原因。
    """
    ...
    if "group_neutralize" in base_alpha or "group_rank" in base_alpha:
        return base_alpha
    # 70% 用 market/sector（粗粒度，自相关低），30% 用 subindustry
    grp = random.choice(["market", "market", "sector", "sector", "industry", "subindustry"])
    return f"group_neutralize({base_alpha}, {grp})"
...
def main(mode: str = "d0", credential_file: str = None):
    ...
    wq_neutralizers = ["market", "sector", "industry", "subindustry"]  # market 优先
    
    # 算子替换映射：平滑算子 → 差分/快变算子（降低自相关）
    _LOW_AUTOCORR_OPS = {
        "ts_decay_linear":  ["ts_delta", "ts_av_diff", "ts_delta"],       # 衰减 → 差分
        "ts_decay_exp_window": ["ts_delta", "ts_av_diff"],                  # 指数衰减 → 差分
        "ts_count_nans":    ["ts_count_nans"],                              # NaN计数保留但允许换窗口
        "ts_backfill":      ["ts_delay", "ts_backfill"],                   # 回填 → 延迟
        "group_rank":       ["group_rank"],                                 # 保持不变
    ...
    def smart_mutate(expr: str) -> str:
        ...
        变异类型（自相关优化版）：
          0.00-0.45 (45%) 字段替换（按类别均匀采样，强制换字段）
          0.45-0.65 (20%) 算子替换（平滑→差分，降低自相关）
          0.65-0.85 (20%) 中性化分组替换（粗粒度优先，降低自相关）
          0.85-1.00 (15%) 参数微调（±30%）
        """
        import re
        mutated = expr
        roll = random.random()
        ...
        if roll < 0.45:
            ...
            # ★ 算子替换：将平滑算子替换为差分算子，直接降低自相关 ★
            for old_op, new_ops in _LOW_AUTOCORR_OPS.items():
                if old_op in mutated and len(new_ops) > 1:
                    new_op = random.choice(new_ops)
                    if new_op != old_op:
                        mutated = mutated.replace(old_op, new_op, 1)
                        # 如果换了算子，也相应调整窗口（差分算子配更短窗口）
                        if new_op in ("ts_delta", "ts_av_diff"):
                            mutated = re.sub(r'(?<!\w)(?:60|90|120|180|360|720)(?!\w)', 
                                           lambda m: str(random.choice([5, 10, 15, 20])), mutated)
                        break  # 一次只换一个算子
                        
        elif roll < 0.85:
            # ★ 中性化分组替换：优先 market/sector（粗粒度=低自相关）★
            for grp_old in ["subindustry", "industry", "sector", "market"]:
                if grp_old in mutated:
                    grp_new = random.choice([g for g in wq_neutralizers if g != grp_old])
                    # 粗粒度优先权重：market 40%, sector 30%, industry 20%, subindustry 10%
                    if random.random() < 0.7:
                        grp_new = random.choice(["market", "sector"])
                    mutated = mutated.replace(grp_old, grp_new, 1)
                    break
            ...
            def tweak(m):
                ...
                if n > 60:  # 大窗口（长周期）更激进地缩短
                    return str(max(10, n - random.randint(20, int(n * 0.6))))
